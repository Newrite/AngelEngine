...
[[maybe_unused]] static void AsClearCallback(asIScriptFunction* Callback)
{
...
[[maybe_unused]] static bool IsAsyncContextPending(asIScriptContext* Context)
{
...
[[maybe_unused]] static bool IsAsyncContextBusy(asIScriptContext* Context)
{
...
public:
	...
	/* AsBasicPromise creation function, for use within AngelScript (Default) */
	static AsBasicPromise* CreateFactoryDefault(asITypeInfo* /*TypeInfo*/)
	{
		return new(asAllocMem(sizeof(AsBasicPromise))) AsBasicPromise(asGetActiveContext());
	}

	/* AsBasicPromise creation function, for use within AngelScript (With Value) */
	static AsBasicPromise* CreateFactoryWithValue(asITypeInfo* /*TypeInfo*/, void* _Ref, int TypeId)
	{
	...
	static AsBasicPromise* CreateFactoryVoid()
	{
		return Create();
	}
	/*
	...
	*/
	static void Register(asIScriptEngine* Engine)
	{
		...
		PROMISE_CHECK(Engine->RegisterObjectBehaviour(PROMISE_TYPENAME "<T>", asBEHAVE_FACTORY, PROMISE_TYPENAME "<T>@ f(int&in)", asFUNCTION(Type::CreateFactoryDefault), asCALL_CDECL));
		PROMISE_CHECK(Engine->RegisterObjectBehaviour(PROMISE_TYPENAME "<T>", asBEHAVE_FACTORY, PROMISE_TYPENAME "<T>@ f(int&in, ?&in)", asFUNCTION(Type::CreateFactoryWithValue), asCALL_CDECL));
...
*/
[[maybe_unused]] static char* AsGeneratePromiseEntrypoints(const char* Text, size_t* InoutTextSize, void*(*AllocateMemory)(size_t) = &asAllocMem, void(*FreeMemory)(void*) = &asFreeMem)
{
